...
<section class="section solutions" id="systems"><div class="container"><div class="solutions-head reveal"><div><span class="eyebrow"><?php echo esc_html( $eyebrow ); ?></span><h2><?php echo esc_html( $heading ); ?></h2></div><p><?php echo esc_html( $lead ); ?></p></div><div class="solution-list">
	<?php foreach ( $rows as $row ) : ?>
		<article class="solution-card reveal"><div class="solution-media"><?php if ( $row['badge'] ) : ?><span class="solution-label"><?php echo esc_html( $row['badge'] ); ?></span><?php endif; ?><?php echo $row['thumbnail']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- from get_the_post_thumbnail(). ?></div><div class="solution-copy"><?php if ( $row['kicker'] ) : ?><span class="kicker"><?php echo esc_html( $row['kicker'] ); ?></span><?php endif; ?><h3><?php echo esc_html( $row['title'] ); ?></h3><?php if ( $row['description'] ) : ?><p><?php echo esc_html( $row['description'] ); ?></p><?php endif; ?><?php if ( $row['capabilities'] ) : ?><div class="solution-capabilities"><?php foreach ( $row['capabilities'] as $capability ) : ?><span><?php echo esc_html( $capability ); ?></span><?php endforeach; ?></div><?php endif; ?><?php if ( $row['tags'] ) : ?><div class="chips" style="margin-top:18px"><?php foreach ( $row['tags'] as $tag ) : ?><span class="chip"><?php echo esc_html( $tag ); ?></span><?php endforeach; ?></div><?php endif; ?><div class="solution-actions"><a href="<?php echo esc_url( $row['permalink'] ); ?>" class="btn btn-primary"><?php esc_html_e( 'View System', 'alrenas' ); ?></a><?php if ( $contact_url ) : ?><a href="<?php echo esc_url( $contact_url ); ?>" class="text-link"><?php esc_html_e( 'Request a quote', 'alrenas' ); ?> <span aria-hidden="true">→</span></a><?php endif; ?></div></div></article>
	<?php endforeach; ?>
</div></div></section>
