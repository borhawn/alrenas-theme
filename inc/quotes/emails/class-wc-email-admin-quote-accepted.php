<?php
/**
 * Class Alrenas_Email_Admin_Quote_Accepted file.
 *
 * @package Alrenas
 */
...
if ( class_exists( 'Alrenas_Email_Admin_Quote_Accepted', false ) ) {
	return new Alrenas_Email_Admin_Quote_Accepted();
}

/**
 * Notifies the shop admin when a customer accepts a sent quote.
 */
class Alrenas_Email_Admin_Quote_Accepted extends WC_Email {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->id             = 'alrenas_admin_quote_accepted';
		$this->title          = esc_html__( 'Quote Accepted (Admin)', 'alrenas' );
		$this->description    = esc_html__( 'Sent to the shop admin when a customer accepts a quote.', 'alrenas' );
		$this->template_html  = 'emails/admin-quote-accepted.php';
		$this->template_plain = 'emails/plain/admin-quote-accepted.php';
		$this->placeholders   = array(
			'{order_date}'   => '',
			'{order_number}' => '',
		);

		add_action( 'woocommerce_order_status_quote-accepted_notification', array( $this, 'trigger' ), 10, 2 );
	...
	public function get_default_subject() {
		return esc_html__( '[{site_title}] Quote #{order_number} accepted', 'alrenas' );
	}
	...
	public function get_default_heading() {
		return esc_html__( 'A customer accepted their quote', 'alrenas' );
	}
